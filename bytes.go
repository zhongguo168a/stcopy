...
func (ctx *Context) toBytes(source Value, provideTyp reflect.Type, w io.Writer, depth int) (err error) {
	...
	case reflect.Map:
		...
		for _, k := range srcref.MapKeys() {
			err = binaryutil.WriteUTF(w, binary.BigEndian, k.String())
...
func (ctx *Context) toBytesProp(kind reflect.Kind, val reflect.Value, w io.Writer) (err error) {
	...
	case reflect.String:
		err = binaryutil.WriteUTF(w, binary.BigEndian, val.String())
	default:
...
func (ctx *Context) fromBytes(source Value, provideTyp reflect.Type, r io.Reader, depth int) (err error) {
	...
	switch provideTyp.Kind() {
	case reflect.Slice, reflect.Array:
		length := int(binaryutil.ReadInt16(r, binary.BigEndian))
		if length == 0 {
			return
		}
		for i := 0; i < length; i++ {
	...
	case reflect.Map:
		...
		for i := 0; i < length; i++ {
			k := reflect.ValueOf(binaryutil.ReadUTF(r, binary.BigEndian))
...
func (ctx *Context) fromBytesProp(kind reflect.Kind, r io.Reader) (result reflect.Value, err error) {
	...
	switch kind {
	case reflect.Bool:
		data = binaryutil.ReadBool(r, binary.BigEndian)
	case reflect.Int:
		data = int(binaryutil.ReadInt64(r, binary.BigEndian))
	case reflect.Int8:
		data = binaryutil.ReadInt8(r, binary.BigEndian)
	case reflect.Int16:
		data = binaryutil.ReadInt16(r, binary.BigEndian)
	case reflect.Int32:
		data = binaryutil.ReadInt32(r, binary.BigEndian)
	case reflect.Int64:
		data = binaryutil.ReadInt64(r, binary.BigEndian)
	case reflect.Uint:
		data = uint(binaryutil.ReadUint64(r, binary.BigEndian))
	case reflect.Uint8:
		data = binaryutil.ReadUint8(r, binary.BigEndian)
	case reflect.Uint16:
		data = binaryutil.ReadUint16(r, binary.BigEndian)
	case reflect.Uint32:
		data = binaryutil.ReadUint32(r, binary.BigEndian)
	case reflect.Uint64:
		data = binaryutil.ReadUint64(r, binary.BigEndian)
	case reflect.Float32:
		data = binaryutil.ReadFloat32(r, binary.BigEndian)
	case reflect.Float64:
		data = binaryutil.ReadFloat64(r, binary.BigEndian)
	case reflect.String:
		data = binaryutil.ReadUTF(r, binary.BigEndian)
	default:
